...
class CaeVecchi_EasyCheckoutBrasil_Model_Type_Geo
{
    ...
    private function _baseData()
    {
        ...
        if (!$customer || !$addresses)
        {
            ...
            if (Mage::getStoreConfig('easycheckoutbrasil/geo_ip/country'))
            {
                $geoip = geoip_open(Mage::getBaseDir('lib').DS.'MaxMind/GeoIP/data/'.Mage::getStoreConfig('easycheckoutbrasil/geo_ip/country_file'),GEOIP_STANDARD);
            ...
            if (Mage::getStoreConfig('easycheckoutbrasil/geo_ip/city'))
            {
                $geoip = geoip_open(Mage::getBaseDir('lib').DS.'MaxMind/GeoIP/data/'.Mage::getStoreConfig('easycheckoutbrasil/geo_ip/city_file'),GEOIP_STANDARD);
            ...
            {
            	$country_id	= Mage::getStoreConfig('easycheckoutbrasil/general/country');
                $result['shipping']['country_id'] = $country_id;
				$result['billing']['country_id'] = $country_id;
            }
        } 
    ...
    public function usePayment($method_code = null)
    {
        ...
        else
        {
            ...
            {
                $method_code = Mage::getStoreConfig('easycheckoutbrasil/general/payment_method');
                foreach ($payments as $payment)
                {
                    if ($method_code !== $payment->getCode())
                        continue;
    ...
    public function useShipping($method_code = null)
    {
        ...
        else
        {
            ...
            {
                $method_code = Mage::getStoreConfig('easycheckoutbrasil/general/shipping_method');
                foreach ($rates['items'] as $rate)
                {
                    if ($method_code === $rate['code'])
                    {
    ...
    public function saveOrder()
    {
        ...
        $service_quote = Mage::getModel('easycheckoutbrasil/service_quote', $this->getQuote());
        if($version == '1.4.0.1')
    ...
    public function validateAddress($address)
    {
        ...
        $address->implodeStreetAddress();
        $a_form = Mage::getStoreConfig('easycheckoutbrasil/address_form');
