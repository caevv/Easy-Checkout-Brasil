<?php
class CaeVecchi_EasyCheckoutBrasil_IndexController extends Mage_Checkout_Controller_Action
{
    ...
    {
        if (!$this->getEasyCheckoutBrasil()->getQuote()->hasItems()
            || $this->getEasyCheckoutBrasil()->getQuote()->getHasError()
            || $this->getEasyCheckoutBrasil()->getQuote()->getIsMultiShipping()) {
    ...
    {$this->_initLayoutMessages('checkout/session');
        if ($this->_current_layout === null)
        {
            ...
            $update->load('easycheckoutbrasil_index_updatecheckout');
    ...
    public function getEasyCheckoutBrasil()
    {
        return Mage::getSingleton('easycheckoutbrasil/type_geo');
    }

    public function indexAction()
    {
        if (!Mage::helper('easycheckoutbrasil')->isEasyCheckoutBrasilEnabled())
        {
        ...
        $quote = $this->getEasyCheckoutBrasil()->getQuote();
        ...
        $this->getEasyCheckoutBrasil()->initDefaultData()->initCheckout();
        $this->loadLayout();
        $this->_initLayoutMessages('customer/session');
        $title	= Mage::getStoreConfig('easycheckoutbrasil/general/title');
    ...
    public function successAction()
    {
        ...
        $session = $this->getEasyCheckoutBrasil()->getCheckout();
    ...
    {
        $lastQuoteId = $this->getEasyCheckoutBrasil()->getCheckout()->getLastQuoteId();
        $lastOrderId = $this->getEasyCheckoutBrasil()->getCheckout()->getLastOrderId();
    ...
    public function getAddressAction()
    {
        ...
        if ($addressId) {
            $address = $this->getEasyCheckoutBrasil()->getAddress($addressId);
    ...
    public function updateCheckoutAction()
    {
        ...
        if ($this->_checkChangedAddress($bill_data, 'Billing', $bill_addr_id) || $this->getRequest()->getPost('payment-method', false))
        {
            ...
            $bill_result = $this->getEasyCheckoutBrasil()->saveBilling($bill_data, $bill_addr_id, false);

            if (!isset($bill_result['error']))
            {
                $pmnt_data = $this->getRequest()->getPost('payment', array());
                $this->getEasyCheckoutBrasil()->usePayment(isset($pmnt_data['method']) ? $pmnt_data['method'] : null);

                $result['update_section']['payment-method'] = $this->_getPaymentMethodsHtml();

                if (isset($bill_data['use_for_shipping']) && $bill_data['use_for_shipping'] == 1 && !$this->getEasyCheckoutBrasil()->getQuote()->isVirtual())
				{
        ...
        if (!$ship_updated && !$this->getEasyCheckoutBrasil()->getQuote()->isVirtual())
        {
            if ($this->_checkChangedAddress($ship_data, 'Shipping', $ship_addr_id) || $ship_method) 
            {
                $ship_result = $this->getEasyCheckoutBrasil()->saveShipping($ship_data, $ship_addr_id, false);
        ...
        $rates = Mage::getModel('sales/quote_address_rate')->getCollection()->setAddressFilter($this->getEasyCheckoutBrasil()->getQuote()->getShippingAddress()->getId())->toArray();
...
// get prev shipping method
		if($check_shipping_diff){
			$shipping = $this->getEasyCheckoutBrasil()->getQuote()->getShippingAddress();
			$shippingMethod_before = $shipping->getShippingMethod();
		}

        $this->getEasyCheckoutBrasil()->useShipping($ship_method);

        $this->getEasyCheckoutBrasil()->getQuote()->collectTotals()->save();
...
///////////////
    ...
    public function saveOrderAction()
    {
        ...
        try {
            ...
            $result = $this->getEasyCheckoutBrasil()->saveBilling($bill_data,$this->getRequest()->getPost('billing_address_id', false));
            if ($result)
            {
            ...
            if ((!$bill_data['use_for_shipping'] || !isset($bill_data['use_for_shipping'])) && !$this->getEasyCheckoutBrasil()->getQuote()->isVirtual())
            {
                $result = $this->getEasyCheckoutBrasil()->saveShipping($this->_filterPostData($this->getRequest()->getPost('shipping', array())),$this->getRequest()->getPost('shipping_address_id', false));
                if ($result)
                {
                	$result['error_messages'] = $result['message'];
                	$result['error'] = true;
            ...
            $agreements = Mage::helper('easycheckoutbrasil')->getAgreeIds();
            if($agreements)
            {
            ...
            if(!isset($result['error']))
            {
                Mage::dispatchEvent('checkout_controller_onepage_save_shipping_method', array('request'=>$this->getRequest(), 'quote'=>$this->getEasyCheckoutBrasil()->getQuote()));
                $this->_subscribeNews();
            }
            ...
            if (!isset($result['redirect']) && !isset($result['error']))
            {
                ...
                if ($pmnt_data)
                    $this->getEasyCheckoutBrasil()->getQuote()->getPayment()->importData($pmnt_data);

                $this->getEasyCheckoutBrasil()->saveOrder();
                $redirectUrl = $this->getEasyCheckoutBrasil()->getCheckout()->getRedirectUrl();
        ...
        {
            Mage::logException($e);
            Mage::helper('checkout')->sendPaymentFailedEmail($this->getEasyCheckoutBrasil()->getQuote(), $e->getMessage());

            $result['error_messages'] = $e->getMessage();
            $result['error'] = true;
            $result['success'] = false;

            $goto_section = $this->getEasyCheckoutBrasil()->getCheckout()->getGotoSection();
            if ($goto_section)
            {
            	$this->getEasyCheckoutBrasil()->getCheckout()->setGotoSection(null);
                $result['goto_section'] = $goto_section;
            }

            $update_section = $this->getEasyCheckoutBrasil()->getCheckout()->getUpdateSection();
            if ($update_section)
            {
                ...
                $this->getEasyCheckoutBrasil()->getCheckout()->setUpdateSection(null);
            }

            $this->getEasyCheckoutBrasil()->getQuote()->save();
        } 
        catch (Exception $e)
        {
            Mage::logException($e);
    ...
    protected function _saveOrderPurchase()
    {
        ...
        {
            $pmnt_data = $this->getRequest()->getPost('payment', array());
            $result = $this->getEasyCheckoutBrasil()->savePayment($pmnt_data);

            $redirectUrl = $this->getEasyCheckoutBrasil()->getQuote()->getPayment()->getCheckoutRedirectUrl();
            if ($redirectUrl)
            {
                $result['redirect'] = $redirectUrl;
            }
    ...
    protected function _checkChangedAddress($data, $addr_type = 'Billing', $addr_id = false)
    {
        ...
        $address = $this->getEasyCheckoutBrasil()->getQuote()->{$method}();
